# Project GIGDIS (beta5.0)

Project GIGDIS 是一个全球热点地图系统的 beta5.0 迭代版本。
...
### beta5.0
- 彻底重做“股市指数显示系统”：后端移除旧的腾讯/新浪抓取链路，改为统一三层通道（`yfinance` → Yahoo Quote API → Stooq）并输出每个指数的实际更新渠道，避免单点故障。
- 指数刷新逻辑重构为配置化模型（指数代码、地区、市值权重、影响系数），同时新增渠道健康统计，便于观测各更新渠道可用性。
- 同步重做“全球经济指数”：后端直接计算并返回 `global_economy` 快照（指数分数、加权涨跌、上涨/下跌家数、地区趋势、贡献排名）。
- 前端指数展示改为完全消费后端实时数据，不再使用本地硬编码指数权重进行二次计算；地图弹窗新增指数更新渠道展示，概览卡与贡献排名与后端统一。
- 新增离线/受限网络兜底：当所有实时行情渠道不可达时，系统自动回退到内置基线指数快照并标记 `update_channel=baseline`，避免前端整屏 `N/A`。
- 全量同步版本号为 `beta5.0`（服务端、前端标题/页眉、README、插件规范、主题文档、依赖注释）。
